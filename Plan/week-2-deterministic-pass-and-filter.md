...
**Goal:** `prr scan <repo>` produces a trustworthy whole-repo markdown report — no phantom lines, no duplicates, no nitpick spam.
...
## Tasks
- [ ] `core/detect_static.py` — run and normalize into `Finding`s:
  - `ruff check --output-format json` → `source="ruff"`
  - `mypy --output json` (or parse text) → `source="mypy"`
  - `bandit -r -f json` → `source="bandit"`
  - (Optional, multi-language later: `semgrep --json`.)
- [ ] `core/context.py` — for each code chunk, attach the static findings landing on it, so the LLM sees them as anchors ("ruff already flagged line 42; reason about logic the tools can't").
- [ ] `core/filter.py` — the trust gate, applied to **all** findings:
...
- [ ] `frontends/cli.py` — add `prr scan <path>`: walk files (respect `ignore` globs) → static pass → LLM pass → filter → write a markdown report grouped by file/severity (with the cat's mood per file).
- [ ] `config.yaml` — `severity_threshold`, `ignore_paths`, `max_comments_per_file`, `model`.

## Deliverables
- Static-analysis runners feeding the same `Finding` schema.
- The filter (line-validation + dedup + threshold + cap).
- `prr scan` producing a clean markdown report.
